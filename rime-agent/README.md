...
|-----|------|---------|-------------|
| ri-detection-resources | object | `{"enabled":false,"modelServers":{"factualInconsistency":{"enabled":false},"factualInconsistencyJa":{"enabled":false},"languageDetection":{"enabled":false},"promptInjection":{"enabled":false},"promptInjectionMulticlass":{"enabled":false},"promptInjectionPrivacy":{"enabled":false},"textEmbedding":{"enabled":false},"toxicityInterpretability":{"enabled":false}}}` | RI detection resources like model servers and YARA servers. These are used for GAI model testing. |
