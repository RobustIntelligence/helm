...
common:
    ...
    connections:
        ...
        addresses:
            generativeTestingDetectionEngineAddr: "{{ include "ai-validation.fullname" . }}-{{ .Values.aiValidation.detectionServer.name }}:{{ .Values.aiValidation.detectionServer.port }}"
        {{- end }}
{{- end }}
...
{{- if .Values.aiValidation.generativeValidationService.serviceAccount.create -}}
    {{ default (printf "%s-%s" (include "ai-validation.fullname" .) .Values.aiValidation.generativeValidationService.name) .Values.aiValidation.generativeValidationService.serviceAccount.name | trunc 63 | trimSuffix "-" }}
{{- else -}}
