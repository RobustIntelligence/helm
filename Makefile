...
#
# To create a new release of the RIME chart (now includes firewall):
...
RI_FIREWALL_YARA_OPERATOR_ROLE_FILE := $(shell pwd)/ri-firewall/templates/yara-operator/role.yaml
...
YARA_OPERATOR_DIR := ../../go/generativefirewall/instancemanager/yaramanager/operator
YARA_CRD_FILENAME := yara-server.crds
...
.rime-releases/ri-firewall-$(VERSION).tgz: .tmp-charts/ri-detection-resources .tmp-charts/ri-firewall
	( \
		$(call check_defined, APP_VERSION VERSION, helm chart version) \
		mkdir -p .rime-releases && \
		pushd .tmp-charts/ && \
		helm package --app-version=$(APP_VERSION) --version=$(VERSION) --destination=../.rime-releases ri-firewall && \
		popd \
	)
...
.rime-releases/index.yaml: .rime-releases/rime-$(VERSION).tgz .rime-releases/rime-agent-$(VERSION).tgz .rime-releases/ri-firewall-$(VERSION).tgz .rime-releases/rime-extras-$(VERSION).tgz .rime-releases/rime-kube-system-$(VERSION).tgz
	( \
		mkdir -p .rime-releases && \
		pushd .rime-releases/ && \
...
# TODO (ENT-3699): add dependence on yara-operator role file
gen_operator_manifests: gen_rime_agent_crds gen_rime_agent_deployment_crds gen_ri_firewall_crds gen_yara_crds $(RIME_AGENT_OPERATOR_ROLE_FILE) $(RIME_AGENT_DEPLOYMENT_OPERATOR_ROLE_FILE) $(RI_FIREWALL_OPERATOR_ROLE_FILE) $(RI_FIREWALL_YARA_OPERATOR_ROLE_FILE)
...
.PHONY: gen_yara_crds
gen_yara_crds: $(wildcard ../../go/generativefirewall/instancemanager/yaramanager/operator/api/v1/*.go) $(RI_FIREWALL_CRD_OUT_DIR)
	$(call generate_crds,$(YARA_OPERATOR_DIR),$(RI_FIREWALL_CRD_OUT_DIR),${YARA_CRD_FILENAME})
...
$(RI_FIREWALL_YARA_OPERATOR_ROLE_FILE): ../../go/generativefirewall/instancemanager/yaramanager/operator/controllers/yaraserver_controller.go
	$(call generate_operator_role,$(RI_FIREWALL_YARA_OPERATOR_ROLE_FILE),$(YARA_OPERATOR_DIR),{{ include "ri-firewall.fullname" . }}-{{ .Values.riFirewall.yaraOperator.name }}-role,.Values.riFirewall.yaraOperator.serviceAccount.create)
...
operator_role_diff_check:
	...
	$(call operator_role_diff_check_impl,$(RI_FIREWALL_YARA_OPERATOR_ROLE_FILE),$(YARA_OPERATOR_DIR),{{ include "ri-firewall.fullname" . }}-{{ .Values.riFirewall.yaraOperator.name }}-role,.Values.riFirewall.yaraOperator.serviceAccount.create)
