...
# TODO(VAL-2913): Remove the `rm` exception for rbst.io_continuoustestjobs.yaml.
crd_diff_check:
	$(eval $@_TMP := $(shell mktemp -d /tmp/crdXXXXXXXXXXXXXXX))
	$(call generate_crds,$($@_TMP))
	-@rm $(CRD_DIR)/rbst.io_continuoustestjobs.yaml
	-@rm $($@_TMP)/rbst.io_continuoustestjobs.yaml
...
# for the given VERSION.
.firewall-releases/ri-firewall-$(VERSION).tgz: .tmp-charts/ri-firewall
	( \
		$(call check_defined, APP_VERSION VERSION, helm chart version) \
		mkdir -p .firewall-releases && \
		pushd .tmp-charts/ && \
		helm package --app-version=$(APP_VERSION) --version=$(VERSION) --destination=../.firewall-releases ri-firewall && \
		popd \
	)

# Rule to update the release index with metadata about release VERSION.
.firewall-releases/index.yaml: .firewall-releases/ri-firewall-$(VERSION).tgz
	( \
		mkdir -p .firewall-releases && \
